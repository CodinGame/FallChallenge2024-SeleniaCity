...
export function getWorkerDispatch(prevWorkers: number[], workers: number[]) {
  const newWorkers = []
  const leavingWorkers = []
  const stayingWorkers = []

  const prevWorkersCountByType = prevWorkers.reduce((acc, w) => {
    if (!(w in acc)) {
      acc[w] = 0
    }
    acc[w]++
  ...
  const workersCountByType = workers.reduce((acc, w) => {
    if (!(w in acc)) {
      acc[w] = 0
    }
    acc[w]++
  ...
  for (const [type, count] of Object.entries(workersCountByType)) {
    if (!(type in prevWorkersCountByType)) {
      for (let i = 0; i < count; i++) {
        newWorkers.push(+type)
      }
    } else {
      for (let i = 0; i < count - prevWorkersCountByType[type]; i++) {
        newWorkers.push(+type)
      }
    }
  }

  for (const [type, count] of Object.entries(prevWorkersCountByType)) {
    if (!(type in workersCountByType)) {
      for (let i = 0; i < count; i++) {
        leavingWorkers.push(+type)
      }
    } else {
      for (let i = 0; i < count - workersCountByType[type]; i++) {
        leavingWorkers.push(+type)
      }
      for (let i = 0; i < Math.min(count, workersCountByType[type]); i++) {
        stayingWorkers.push(+type)
      }
    }
  }

  return {newWorkers, leavingWorkers, stayingWorkers}
}